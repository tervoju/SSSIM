...
//       printf ("pool initialised\n");    
     ...
     printf("ERROR! Array %s size of the block is strange: %d!\n", arrayname, array_size);
	 return NULL;
   }
...
{
	if (row == column) return number;
	if (row == column-num_units * 3) return number * delta_t;
...
MTYP swatrix_multiply_C_component(const MTYP number, const int row, const int column, 
								   const C_matr C)
{
	if (column >= C.diag_length) return 0;

	if (row == column)
		return number * C.diag_vector[row];

	if (row < C.diag_length) return 0;
